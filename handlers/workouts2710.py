...
async def handle_conflicting_finish(callback: CallbackQuery, state: FSMContext):
    """Обработка конфликтующих finish callback"""
    logger.info(f"🔄 Перенаправляем конфликтующий callback: {callback.data}")
    # Перенаправляем на правильную функцию
    await finish_create_workout(callback, state)
...
async def finish_workout(callback: CallbackQuery, state: FSMContext):
    ...
    if n is None:
        ...
        if callback.data in ("finish_workout_creation", "create_finish_workout"):
...
async def rename_workout(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(CreateWorkoutStates.renaming_workout)
    await callback.answer()
...
async def change_desc_workout(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(CreateWorkoutStates.changing_workout_description)
    await callback.answer()
...
async def process_workout_text_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        if current_state in (
            ...
            CreateWorkoutStates.searching_exercise_for_block.state,
        ...
        # renaming workout
        if current_state == CreateWorkoutStates.renaming_workout:
        ...
        if current_state == CreateWorkoutStates.changing_workout_description:
            new = message.text.strip()
            d = await state.get_data()
            wid = d.get('editing_workout_id')
            if not wid:
        ...
        if current_state == "waiting_rpe":
            try:
                ...
                await state.set_state(CreateWorkoutStates.waiting_weight)
            except ValueError:
                await message.answer("❌ Введите число от 1 до 10.")
            return

        # waiting for weight after workout finish
        if current_state == CreateWorkoutStates.waiting_weight:
            d = await state.get_data()

...
# === ПОКАЗАТЬ УПРАЖНЕНИЯ ПО КАТЕГОРИИ ===
async def show_category_exercises(callback: CallbackQuery, state: FSMContext):
    category = callback.data[len("cat_"):]

    async with db_manager.pool.acquire() as conn:
        exercises = await conn.fetch(
            "SELECT id, name, muscle_group, test_type FROM exercises WHERE LOWER(category) = LOWER($1) ORDER BY name LIMIT 20",
            category
        )
    
    if not exercises:
        await callback.message.edit_text(f"Упражнений в категории *{category}* не найдено.", parse_mode="Markdown")
        await callback.answer()
        return

    text = f"**Категория: {category}** ({len(exercises)} упр.)\n\n"
    kb = InlineKeyboardBuilder()
    
    current_state = await state.get_state()
    is_in_workout = current_state == CreateWorkoutStates.searching_exercise_for_block

    for ex in exercises:
        emoji = {
            'strength': 'Weight', 'endurance': 'Timer', 'speed': 'Runner',
            'quantity': 'Numbers', 'none': 'Muscle'
        }.get(ex['test_type'] or 'none', 'Muscle')
        
        if is_in_workout:
            kb.button(text=f"{emoji} {ex['name']}", callback_data=f"use_in_workout_{ex['id']}")
        else:
            kb.button(text=f"{emoji} {ex['name']}", callback_data=f"exercise_{ex['id']}")

    if is_in_workout:
        kb.button(text="Новый поиск", callback_data="workout_start_search")
        kb.button(text="Назад к блоку", callback_data="create_back_to_blocks")
    else:
        kb.button(text="К категориям", callback_data="search_by_category")
...
# === ПОКАЗАТЬ УПРАЖНЕНИЯ ПО ГРУППЕ МЫШЦ ===
async def show_muscle_group_exercises(callback: CallbackQuery, state: FSMContext):
    muscle = callback.data[len("muscle_"):]

    async with db_manager.pool.acquire() as conn:
        exercises = await conn.fetch(
            "SELECT id, name, category, test_type FROM exercises WHERE LOWER(muscle_group) = LOWER($1) ORDER BY name LIMIT 20",
            muscle
        )
    
    if not exercises:
        await callback.message.edit_text(f"Упражнений для *{muscle}* не найдено.", parse_mode="Markdown")
        await callback.answer()
        return

    text = f"**Группа мышц: {muscle}** ({len(exercises)} упр.)\n\n"
    kb = InlineKeyboardBuilder()
    ...
    for ex in exercises:
        ...
        if is_in_workout:
            kb.button(text=f"{emoji} {ex['name']} ({ex['category']})", callback_data=f"use_in_workout_{ex['id']}")
        else:
            kb.button(text=f"{emoji} {ex['name']} ({ex['category']})", callback_data=f"exercise_{ex['id']}")

    if is_in_workout:
        kb.button(text="Новый поиск", callback_data="workout_start_search")
        kb.button(text="Назад к блоку", callback_data="create_back_to_blocks")
    else:
        kb.button(text="К группам мышц", callback_data="search_by_muscle")
    kb.button(text="Главное меню", callback_data="main_menu")
    kb.adjust(1)
...
async def show_muscle_group_exercises(callback: CallbackQuery):
    ...
    try:
        ...
        if exercises:
            ...
            is_in_workout = current == CreateWorkoutStates.searching_exercise_for_block
            
            for ex in exercises:
                test_emoji = {
                    'strength': 'Weight', 'endurance': 'Timer', 'speed': 'Runner',
                    'quantity': 'Numbers', 'none': 'Muscle'
                }.get(ex['test_type'] if ex['test_type'] else 'none', 'Muscle')
                
                if is_in_workout:
                    keyboard.button(
                        text=f"{test_emoji} {ex['name']} ({ex['category']})", 
                        callback_data=f"use_in_workout_{ex['id']}"
                    )
                else:
                    keyboard.button(
                        text=f"{test_emoji} {ex['name']} ({ex['category']})", 
            ...
            if is_in_workout:
                keyboard.button(text="Новый поиск", callback_data="workout_start_search")
                keyboard.button(text="Назад к блоку", callback_data="create_back_to_blocks")
            else:
